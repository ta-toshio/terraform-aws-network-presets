...
terraform {
  ...
  backend "s3" {
    bucket               = "tf-state-myproject-production"
    key                  = "tf-state-deploy"
    workspace_key_prefix = "tf-state-deploy-env"
    region               = "ap-northeast-1"
    encrypt              = true
    dynamodb_table       = "tf-lock-myproject-production"
  }
}
...
module "ecs" {
  ...
  min_capacity       = local.web_min_capacity
  max_capacity       = local.web_max_capacity
...
module "amplify" {
  ...
  branch_name    = "production" # ステージング環境用のブランチ名
